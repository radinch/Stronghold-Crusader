...
public class DropMenuController {
    ...
    public String setTileTexture(Matcher matcher) {
        ...
        DataBank.putTexture(map.getMapCells(),x,x + 1,y,y + 1,texture);
        return "successful";
    }
    ...
    private boolean isThereABuilding(int startX,int endX,int startY, int endY) {
        for (int i = startX; i < endX ; i++) {
            for (int j = startY; j <endY ; j++) {
                if (map.getACell(i,j).getBuilding() != null)
                    return true;
            }
        }
        return false;
